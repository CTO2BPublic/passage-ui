...
const AccessRequestFormModal: React.FC<Props> = ({ open, handleClose, initialRoleName = '' }) => {
    ...
    >
      <Stack sx={style} gap={4}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography id="access-request-modal-title" variant="h6" component="h2">
            New Access Request
          </Typography>
          <IconButton onClick={handleClose} aria-label="Close access request form">
            <CloseIcon />
          </IconButton>
        </Stack>
          ...
          <form
            onSubmit={methods.handleSubmit((values) => {
              mutate(values, { onSuccess: () => {
                if (location.pathname !== paths.accessRequests)         navigate(paths.accessRequests);
                  handleClose();
                } });
            })}
          >
            <Stack spacing={2}>
              <ControlledSelectField
                ...
                size="small"
                disabled={isRolesLoading || isSubmitLoading || !!initialRoleName}
